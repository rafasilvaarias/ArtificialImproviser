"""
Note collection and recording module
"""
...
class NoteRecorder:
    def __init__(self, agent=None, enable_agent=False):
        self.notes = []  # List of completed notes
        self.current_note = None  # Currently recording note
        self.last_record_time = None  # Time of last recorded point
        self.pause_start_time = None
        self.POINT_INTERVAL = 0.0002  # Record points every 0.2ms
        # New phrase-related state
        self.phrase_num = 1
        self.PAUSE_PHRASE_THRESHOLD = 3.0  # trigger end of phrase after 10s pause
        self.LAST_NOTE_PAUSE = 3.0          # assign 3s pause to the last note when triggered
        self.pause_triggered = False
        self.last_phrase_ended = None  # Track which phrase just ended (for generative agent)
        # Agent reference for setting hotness
        self.agent = agent
        self.enable_agent = enable_agent
        
    def start_note(self, fingers, x, y, z, angle, velocity, timestamp):
        """Start a new note (save previous note if exists)"""
        # Save previous note if it exists
        if self.current_note is not None:
            self.save_current_note()

        # If we just saved a previous note, set its pause_after now
        if self.notes:
            last_note = self.notes[-1]
            # Only set pause_after if we haven't already (avoid overwriting phrase-end value)
        ...
        # Reset pause timer when a new note starts (only consecutive pauses count)
        self.pause_start_time = None
        self.pause_triggered = False
        ...
        self.current_note = {
            ...
            'data_points': [(x, y, z, angle, velocity, 0)],  # First point at time=0
    ...
    def record_point(self, x, y, z, angle, velocity, timestamp):
        """Record a data point for the current note (every 0.2ms)"""
        if self.current_note is not None:
    ...
    def pause(self, timestamp):
        """Record a pause (no fingers pinched). Trigger phrase-end after sustained pause."""
        if self.current_note is not None:
            self.save_current_note()
        ...
        if self.pause_start_time is None:
            ...
            #if pause_duration % 2.0 < 0.05 or pause_duration >= self.PAUSE_PHRASE_THRESHOLD:
                #print(f"[DEBUG] Pause: {pause_duration:.2f}s / {self.PAUSE_PHRASE_THRESHOLD}s")
            if pause_duration >= self.PAUSE_PHRASE_THRESHOLD and not self.pause_triggered:
                #print(f"[DEBUG] >>> THRESHOLD REACHED! Calling end_phrase()...")
                self.end_phrase(timestamp)
                self.pause_triggered = True

    def end_phrase(self, timestamp=None):
        """Assign phrase number to any notes without it and set last-note pause to 3s."""
        print(f"\n{'='*80}")
        ...
        for i, note in enumerate(self.notes):
            ...
            #else:
                #print(f"[PHRASE_END]   Note {i}: already has phrase {note['phrase']}")
        ...
        for i in range(len(self.notes) - 1):
            ...
            #print(f"[PHRASE_END]   Note {i} <-> Note {i+1} similarity: {sim:.4f}")
        
        phrase_cohesion = np.mean(similarities) if similarities else 0.0
        print(f"[PHRASE_END] ////////////////////////// Cohesion {phrase_cohesion} ")

        # Set hotness of generative agent based on cohesion; 0.4 - 0.8 cohesion is 1 - 0 hotness.
        if self.enable_agent and self.agent is not None:
            hotness = max(0.0, min(1.0, 2 * (0.8 - phrase_cohesion)))  # Linear mapping
        ...
        #print(f"[PHRASE_END] Next phrase will be #{self.phrase_num}")
        #print(f"{'='*80}\n")
        
        # Store the phrase that just ended so main.py can trigger generative agent
        self.last_phrase_ended = phrase_num_ended
        
    def save_current_note(self):
        """Save the current note to completed notes if it's at least 0.1s long"""
        if self.current_note is not None:
            duration = self.current_note['timestamps'][-1] - self.current_note['start_time']
            self.current_note['duration'] = duration
            
            # Initialize pause_after if not set (will be calculated later in finalize)
            if 'pause_after' not in self.current_note:
                self.current_note['pause_after'] = 0.0
            
            # Only save notes that are at least 0.1 seconds long
            if duration >= 0.1:
                self.notes.append(self.current_note)
                #print(f"[DEBUG] SAVE_NOTE: fingers={self.current_note['fingers']}, duration={duration:.3f}s -> SAVED (#{len(self.notes)})")
            #else:
                #print(f"[DEBUG] SAVE_NOTE: duration={duration:.3f}s -> REJECTED (too short)")
            
            self.current_note = None
            self.frame_count = 0
    
    def finalize(self, current_time):
        """Call when program ends to save current note and compute pauses.
           NOTE: phrase assignment is handled during recording via end_phrase()."""
        # Save last note
        if self.current_note is not None:
            self.save_current_note()
        
        # Calculate pause durations between notes (keeps pause_after up-to-date)
        for i in range(len(self.notes) - 1):
            end_time = self.notes[i]['timestamps'][-1]
            next_start_time = self.notes[i + 1]['start_time']
            if self.notes[i].get('pause_after', 0.0) == 0.0:  # Only set if not already assigned (e.g. phrase-end)
                self.notes[i]['pause_after'] = next_start_time - end_time

        # Ensure source field is present for all notes
        for note in self.notes:
            if 'source' not in note:
                note['source'] = 'human'

        # DEBUG: Print note structure
    ...
    def note_similarity(self, note1, note2):
        """Compare notes based on finger configuration and trajectory shape"""
        ...
        # Calculate the total path length of each trajectory
        ...
        # Compare path lengths (similar gestures should have similar total movement)
        if path1 > 0 and path2 > 0:
            path_ratio = min(path1, path2) / max(path1, path2)
        else:
            path_ratio = 0.0
        
        # Calculate directional similarity (are they moving in similar directions?)
        ...
        if norm1 > 0 and norm2 > 0:
            ...
            direction_sim = (direction_sim + 1) / 2  # Map from [-1,1] to [0,1]
        else:
            direction_sim = 0.0
        
        # Combine: fingers (30%), path length (30%), direction (40%)
